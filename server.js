...
const loginLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 10,
  message: { error: 'Demasiados intentos de inicio de sesi├│n. Intenta de nuevo en 15 minutos.' },
...
const apiLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 100,
  message: { error: 'Demasiadas solicitudes. Intenta de nuevo m├ís tarde.' },
  standardHeaders: true,
  legacyHeaders: false,
});
...
app.post('/api/admin/sync', apiLimiter, verifyAdmin, async (req, res) => {
  ...
  if (!folderId || folderId.includes('your_drive_folder_id')) {
    return res.status(400).json({ error: 'La ID de la carpeta de Google Drive no est├í configurada en el archivo .env' });
  }
...
async function startServer() {
  try {
    ...
    app.listen(PORT, () => {
      console.log(`\n======================================================`);
      console.log(`­ƒÜÇ Chatbot Servidor corriendo en puerto: ${PORT}`);
      console.log(`­ƒûÑ´©Å  Panel de Admin disponible en: http://localhost:${PORT}`);
      console.log(`­ƒñû Google Chat Webhook en: http://localhost:${PORT}/api/chat`);
      console.log(`======================================================\n`);
    });
  } catch (err) {
